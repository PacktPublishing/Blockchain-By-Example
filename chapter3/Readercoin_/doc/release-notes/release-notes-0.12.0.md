Readercoin Core version 0.12.0 is now available from:

  <https://readercoin.org/bin/readercoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Readercoin-Qt (on Mac) or
readercoind/readercoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Readercoin Core or other software:
...
pre-0.12 versions of Readercoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Readercoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Readercoin transactions now use validation using
[libsecp256k1](https://github.com/readercoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/readercoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Readercoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Readercoin Core was to raise the default minimum
relay fee.

Readercoin Core 0.12 will have a strict maximum size on the mempool. The
...
Readercoin Core 0.12 also introduces new default policy limits on the length and
...
Readercoin Core 0.12 nodes. Readercoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/readercoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Readercoin Core 0.12 does not yet have support for
...
This allows running readercoind without having to do any manual configuration.
...
Readercoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Readercoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Readercoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Readercoin Core 0.13, and it is at this time undecided whether the
...
Readercoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Readercoin Core automatically creates a hidden service to listen on, without
manual configuration. Readercoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Readercoin Core is listening, and
...
Readercoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Readercoin
Core to use floating fees. By default, Readercoin Core will use floating
fees.
...
(default: `0.0002` RDC/kB).

At all times, Readercoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) RDC.
Furthermore, Readercoin Core will never create transactions paying less than
...
`readercoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/readercoin/readercoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/readercoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/readercoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /readercoinrpc>
        ...
        # optional bypass readercoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< readercoinrpc:<password>
    </Location>
...
Restarting readercoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Readercoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new readercoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include readercoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix readercoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of readercoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override READERCOIND and READERCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [readercoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add READERCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libreadercoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "RDC" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/readercoin/).
