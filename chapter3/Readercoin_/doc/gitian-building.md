...
*Setup instructions for a Gitian build of Readercoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Readercoin
...
- [Building Readercoin Core](#building-readercoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-readercoin-host/README.md).
...
Clone the git repositories for readercoin and Gitian.
...
in the readercoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Readercoin Core
----------------

To build Readercoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the readercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/readercoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/readercoin/readercoin
...
For example:
```bash
URL=https://github.com/laanwj/readercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit readercoin=${COMMIT} --url readercoin=${URL} ../readercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit readercoin=${COMMIT} --url readercoin=${URL} ../readercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit readercoin=${COMMIT} --url readercoin=${URL} ../readercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the readercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../readercoin/contrib/gitian-descriptors/*|sort|uniq )
...
RDCPATH=/some/root/path/readercoin
SIGPATH=/some/root/path/readercoin-detached-sigs

./bin/gbuild --url readercoin=${RDCPATH},signature=${SIGPATH} ../readercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/readercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/readercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/readercoin-osx-build.assert
```
...
[readercoin-core/gitian.sigs](https://github.com/readercoin-core/gitian.sigs/) repository, or if that's not possible create a pull
