...
from test_framework.test_framework import ReadercoinTestFramework, SkipTest
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                )

class ZMQTest (ReadercoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that readercoin has been built with ZMQ enabled
        ...
        if not config["components"].getboolean("ENABLE_ZMQ"):
            raise SkipTest("readercoind has not been built with zmq enabled.")
