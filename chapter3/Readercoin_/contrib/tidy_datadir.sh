#!/bin/bash
# Copyright (c) 2013 The Readercoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Readercoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Readercoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Readercoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Readercoin 0.7 datadir."
    ;;
  3)
    echo "Detected Readercoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Readercoin 0.8 datadir."
    ;;
esac
