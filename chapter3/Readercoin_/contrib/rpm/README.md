...
The RPM spec file provided here is for Readercoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://readercoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Readercoin git repository](https://github.com/readercoin/readercoin) but are not part of
...
Currently only `Source10` is used, for the example `readercoin.conf` file.
...
in the readercoin git repository. At present only an SVG version of the Readercoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the readercoin
...
needed when 0.12.1 is released, a proper fix is already in the Readercoin git
master branch.
...
idea to build Readercoin Core against a static version of that library compiled
...
The `readercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `readercoin-server` package. At this point, most RPM based
...
The `readercoin-server` package should have SELinux support. How to properly do
...
Readercoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/readercoin/). He has been
packaging Readercoin for Fedora at least since 2012.
...
4. I renamed the `readercoin` package that contains the Qt GUI to `readercoin-core` as
that appears to be how the general population refers to it, in contrast to
`readercoin-xt` or `readercoin-classic`. I wanted to make sure the general population
