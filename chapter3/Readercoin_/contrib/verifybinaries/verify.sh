...
###   readercoincore.org and readercoin.org and compares them.
...
WORKINGDIR="/tmp/readercoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://readercoincore.org"
HOST2="https://readercoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="readercoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'readercoin-',
...
if [ $? -ne 0 ]; then
   echo "readercoin.org failed to provide signature file, but readercoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "readercoin.org and readercoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Readercoin Core binary release signing key installed?"
   fi
